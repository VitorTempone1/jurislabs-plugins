...
O texto do advogado sai em português, e este guia foi escrito para o inglês. Ao trabalhar em português:

- **Aplique os conceitos, não as listas de palavras.** As listas dos §7, §26 e §32 são vocabulário inglês. Os padrões por trás delas (inflar importância, evitar "é", regra de três, aforismo pronto) acontecem igual em português: "desempenha um papel fundamental", "não se trata apenas de X, mas de Y", "consolidando-se como", "em um cenário cada vez mais".
- **§14 (travessão) vale integral.** O travessão (—) é o sinal mais confiável de texto de IA e o texto final não pode ter nenhum. Em português troque por ponto, vírgula, dois-pontos ou parênteses.
- **§17 (title case) não se aplica.** Português já usa caixa de sentença nos títulos.
- **Peça jurídica não é blog.** A seção PERSONALITY AND SOUL fica **desligada** em petição, parecer, contrato e e-mail formal para cliente: ali o neutro e direto já é a voz humana correta. Ela só liga em post, artigo e texto de captação.
- **Não mexa em citação.** Transcrição de lei, ementa, súmula e trecho de decisão são literais: nunca reescreva o que está entre aspas ou em bloco de citação.
